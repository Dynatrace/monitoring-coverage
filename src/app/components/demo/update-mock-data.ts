...
export function updateMockData(queryClient: QueryClient, cloudType: CloudType) {
  ...
  const unmonitoredHosts = generateHostData(
    hostStatus.hosts - demoHosts,
    "NEWCLOUDHOST",
    "newcloud"
  );

  queryClient.setQueryData<CloudHostStatus>(['host-status', cloudType, { demoMode: true }], hostStatus);
